...
class Spider(Spider):
	...
	def homeContent(self, filter):
		...
		r = requests.get(url, params=params, headers=header, timeout=30, proxies={"http": self.proxy, "https": self.proxy})
	...
	def playerContent(self, flag, pid, vipFlags):
		...
		r = requests.post(url, params=params, data=data, headers=header, timeout=30, proxies={"http": self.proxy, "https": self.proxy})
		url = self.baseUrl + r.json()['MediaSources'][0]['DirectStreamUrl']
		
		if int(self.thread) > 0:
			try:
				self.fetch('http://127.0.0.1:7777', timeout=30)
			except:
	...
	def getAccessToken(self):
		...
		header = self.header.copy()
		header['Content-Type'] = "application/json; charset=UTF-8"
		r = requests.post(f"{self.baseUrl}/emby/Users/AuthenticateByName", params={"Username": self.username, "Password": self.password , "Pw": self.password , "X-Emby-Client": "SenPlayer", "X-Emby-Device-Name": "SenPlayer", "X-Emby-Device-Id": str(uuid4()), "X-Emby-Client-Version": "4.2.1"}, headers=header, timeout=30, proxies={"http": self.proxy, "https": self.proxy})
		embyInfos = r.json()
		self.setCache(key, embyInfos)
		return embyInfos

	header = {"User-Agent": "SenPlayer/1 CFNetwork/1312 Darwin/21.0.0"}
